...
public interface ArticleDao extends JpaRepository<Article, Long>, JpaSpecificationExecutor<Article> {
    ...
    @Query(value = "select t1.* from tic.tbl_article_list t1 left join tic.tbl_article_tag t2 on t1.id=t2.article_id where t1.id=?1 order by t1.id desc", nativeQuery = true)
    List<Article> findEventTrackByArticleId(long articleId);

    @Modifying
    @Transactional
    @Query(value = "update tic.tbl_article_list set vote_positive_count=vote_positive_count+1 where id=?1", nativeQuery = true)
    int updateVotePositiveCountById(long id);

    @Modifying
    @Transactional
    @Query(value = "update tic.tbl_article_list set vote_negtive_count=vote_negtive_count+1 where id=?1", nativeQuery = true)
